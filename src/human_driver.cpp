...
double HumanDriver::h_dot(double vel_1)
{
    ...
    return hdot = vel_1 - current_velocity_;
}
...
double HumanDriver::v_dot(double vel_1, double headway)
{
    ...
    vdot = alpha*(velocity(headway) - current_velocity_);
    return vdot + beta * (vel_1 - current_velocity_);
}
...
double HumanDriver::range_policy(double h)
{
    return range_policy_.velocity(h);
}
